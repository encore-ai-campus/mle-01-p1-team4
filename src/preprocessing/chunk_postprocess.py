...
def load_chunks() -> list[dict]:
    if not SOURCE_CHUNKS_PATH.exists():
        raise FileNotFoundError(
            f"입력 파일을 찾을 수 없습니다: "
            f"{SOURCE_CHUNKS_PATH.resolve()}"
        )

    try:
        chunks = json.loads(
            SOURCE_CHUNKS_PATH.read_text(
                encoding="utf-8"
            )
        )
...
def save_chunks(
    ...
    NORMALIZED_CHUNKS_PATH.parent.mkdir(
        parents=True,
        exist_ok=True,
    )
...
def main() -> None:
    ...
    normalized_chunks = postprocess_chunks(
        source_chunks
    )
    ...
    print(
        "저장 위치: "
        f"{NORMALIZED_CHUNKS_PATH.resolve()}"
    )
